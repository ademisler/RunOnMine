...
identity=$(/usr/bin/codesign -dv --verbose=4 "$app" 2>&1 | sed -n 's/^Authority=//p' | head -n1)
...
for binary in runonmine runonmine-agent runonmine-desktop runonmine-helper; do
  /usr/bin/lipo "$app/Contents/MacOS/$binary" -verify_arch arm64 x86_64
  /usr/bin/codesign --verify --strict "$app/Contents/MacOS/$binary"
done
