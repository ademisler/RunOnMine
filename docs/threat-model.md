...
| A forced macOS LaunchAgent restart strands a separately supervised tunnel process group | official RunOnMine start/restart/install/uninstall paths use bounded `SIGTERM` shutdown and wait for the agent PID to disappear before bootout/bootstrap or kickstart; routine lifecycle never uses `launchctl kickstart -k`, and operators are warned not to bypass the service commands |
