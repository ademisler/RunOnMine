...
| A managed Cloudflare update activates a binary but config save or service restart fails | immutable preparation and probing precede mutation; config replacement, active-manifest switch and running-agent restart are one rollback-aware transaction; failure restores the prior config and active version while preserving version artifacts |
