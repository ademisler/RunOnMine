...
| An owner enables full workstation authority through the public Named Tunnel and the remote identity is compromised | explicit off-by-default `owner_full_access` only on GitHub-owner-authenticated Cloudflare OAuth, explicit Full policy, OAuth scope intersection, selected roots, tamper-evident audit, separate helper installation/hash-pinned command profile, visible dangerous UI/CLI wording, and Emergency Lock; the owner accepts that this mode intentionally removes the generic remote ceiling |
...
| Equivalent normal and verbatim Windows paths bypass or break selected-root authorization | normalize only absolute drive/UNC prefixes to one verbatim identity before the existing lexical root check; traversal, device namespaces, reparse points, and out-of-root paths remain denied |
| Windows helper path validation either accepts DLL/replacement risk or rejects every standard system path | reject low-privilege writes on the executable and its direct DLL-search directory, reject delete/DACL/owner mutation on all higher ancestors, retain reparse-point and trusted-owner checks, and exercise the standard System32 path in native acceptance |
...
| A hardened Linux user service sees root-owned pinned binaries as overflow UID/GID values | compare ownership through the kernel's current `uid_map`/`gid_map` and overflow IDs while keeping canonical path, SHA-256, size, modification time, and mode exact; mapped current-user replacements still fail |
| A lingering Linux user manager exposes `XDG_RUNTIME_DIR` but no usable Secret Service | explicit `CREDENTIALS_DIRECTORY` or `RUNONMINE_MASTER_KEY` selects the encrypted file backend; user-service install persists a private validated systemd credential source |
| Headless and desktop DEBs overwrite the same binaries without a defined transition | canonical package identities plus mutual `Conflicts`/`Replaces`, explicit runtime dependencies, metadata inspection, and upgrade/replacement acceptance |
| A second Linux desktop process creates competing tray/window state or replaces an unsafe lock path | owner-private mode-0600 Unix socket, show-primary command, owned-socket stale recovery, and fail-closed handling of regular files, symlinks, or foreign ownership |
| Emergency lock rotates a Quick Tunnel secret but leaves a discovered runtime URL record | configured Quick runtime records are cleared during lock and acceptance verifies no public URL remains before restart |
| Emergency lock rotates a Local HTTP token but a running agent keeps accepting the old value | Local HTTP authentication re-reads the shared secret store on every request, validates the current token, and fails closed when the secret is missing, invalid, or rotated |
...
| A forced macOS LaunchAgent restart strands a separately supervised tunnel process group | official RunOnMine stop/restart/install/uninstall paths unload the job with `launchctl bootout`, the plist provides a 20-second exit window for supervisor cleanup, and start uses `bootstrap` or non-forcing `kickstart`; routine lifecycle never uses `launchctl kickstart -k`, and operators are warned not to bypass the service commands |
...
| A timed-out command, service SIGTERM, or dropped connector supervisor leaves descendants running | Unix agents route SIGINT/SIGTERM through graceful connector teardown; Unix process groups and Windows Job Objects terminate the process tree; dropped handles signal shutdown and detach the cleanup task instead of aborting it |
...
| A Cloudflare Named Tunnel renders `/mcp` into the loopback origin and cloudflared exits before connecting | Named Tunnel origin validation requires a root path and YAML rendering serializes only the URL origin/authority with no path or trailing slash; regression coverage rejects path-bearing origins |
| A managed connector is reported ready merely because its process spawned | a pre-spawn supervisor event receiver drives starting/readiness/backoff/degraded state; only a healthy readiness event can publish `ready` |
| macOS LaunchAgent uses a different ad-hoc binary identity than the CLI that created Keychain credentials | the service stages the canonical `runonmine` CLI bytes and invokes the immutable copy as `agent run`, so CLI and agent share the installed build identity while versioned-byte drift remains rejected |
...
| A public MCP URL is known and an attacker tries to impersonate a pre-provisioned ChatGPT OAuth client | confidential clients are local-owner provisioned only on the explicit owner-full connector, bind exact HTTPS redirect URIs and scopes, require a 256-bit client secret at token exchange, store only a domain-separated keyed hash, retain PKCE plus immutable GitHub owner-ID verification and explicit consent, and cascade credentials on client deletion |
...
| A consent page double-click or browser retry submits the same approval twice | the consent page uses a native HTML form with no client-side submit interception, while a bounded 30-second in-memory consent replay keyed by ID, CSRF hash, and decision returns the original redirect without issuing a second authorization code; mismatches fail closed |
| A strict MCP client derives resource-specific OAuth discovery URLs and fails closed before authorization | `/mcp` challenges advertise `/.well-known/oauth-protected-resource/mcp`; the authorization server publishes identical issuer-bound metadata at the root and `/mcp` well-known aliases, both explicitly advertising PKCE `S256` |
