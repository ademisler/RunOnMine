...
| A consent page double-click or browser retry submits the same approval twice | same-origin single-submit JavaScript disables repeat submission, and a bounded 30-second in-memory consent replay keyed by ID, CSRF hash, and decision returns the original redirect without issuing a second authorization code; mismatches fail closed |
