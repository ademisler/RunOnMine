...
- [!] **P2-R03 — Complete clean-install acceptance on every artifact/OS.** Fresh hosted Linux x86/ARM, Linux desktop x86_64, macOS universal and Windows artifact preflight builds validate checksums/SBOM, setup, agent health, MCP initialize, an owner-approved write, desktop launch where applicable, uninstall and residue cleanup. A physical Apple-silicon Mac additionally verifies both universal slices through Rosetta/native launch, DMG installation, LaunchAgent stop/start, all desktop navigation views, retained-data uninstall and full purge; a physical Ubuntu 24.04 x86_64 Xfce host verifies the full desktop-control build, four-binary SBOM/archive, standalone DEB install/remove/residue cycle, installed MCP lifecycle, Xvfb launch, and a real `IsViewable` Xfce window. The strict release evidence gate still requires an actual OS reboot plus Developer ID signing and notarization.
