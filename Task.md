...
- [!] **P2-R03 — Complete clean-install acceptance on every artifact/OS.** The Linux matrix is complete: exact-commit Ubuntu 24.04 artifacts are exercised in clean x86_64 and ARM64 QEMU guests, including package upgrade/replacement, user and system service recovery across reboot, MCP authorization boundaries, managed Quick Tunnel recovery, uninstall/residue checks, and the x86_64 desktop package on a real X11/D-Bus session for native window, close-to-tray, tray reopen, and single-instance behavior. The remaining cross-platform release gates are Windows native clean-install evidence with publisher signing and macOS clean-install evidence with Apple Developer ID signing/notarization; GNU/Wine and unsigned artifacts remain supplemental only.
