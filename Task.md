...
- [!] **P2-R03 — Complete clean-install acceptance on every artifact/OS.** Fresh hosted Linux x86/ARM, macOS universal and Windows artifact preflight builds validate checksums/SBOM, setup, agent health, MCP initialize, an owner-approved write, desktop launch where applicable, uninstall and residue cleanup. The strict release evidence gate still requires an actual OS reboot and signed/notarized artifact evidence, which hosted one-shot jobs cannot fabricate.
