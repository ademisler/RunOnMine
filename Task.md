...
- [x] **P2-R02 — Keep publisher signing optional and fail-closed when configured.** The macOS beta path can ad-hoc seal unsigned bundles with hardened runtime and sealed resources; when Developer ID/App Store Connect credentials are configured, the separate path signs every Mach-O, notarizes and staples the universal application, signs the DMG, and verifies the result. Windows Authenticode remains optional publisher-trust hardening. Public beta may ship explicitly unsigned; no artifact may claim publisher trust it did not verify.
- [x] **P2-R03 — Implement and exercise clean-install acceptance on every artifact/OS.** The owner-controlled macOS universal, Linux x86_64 headless/desktop, Linux ARM64 headless, and Windows x86_64 procedures all have real reboot, MCP approval/deny, native desktop lifecycle where applicable, helper-boundary, uninstall, and residue coverage. Candidate readiness is intentionally not encoded in this checklist: only `acceptance/release-candidate.toml`, `acceptance/release-gates.toml`, and matching machine-readable evidence determine whether the current freeze may be tagged.
...
- [x] Exact-candidate evidence is accepted only through the machine-readable release manifest and gates; historical macOS/Linux/Windows reports are never reused for a newer freeze. Owner-controlled macOS build/package, CLI/MCP, and desktop smoke evidence remains supplemental until the FileVault-protected physical reboot cycle is completed by the owner. Publisher signing/notarization and independent security review remain optional hardening; GitHub-hosted execution and protected-main enforcement remain required public-beta gates and are not fabricated by repository CI.
