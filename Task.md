...
- [!] **P2-R03 — Complete clean-install acceptance on every artifact/OS.** Fresh hosted Linux x86/ARM, Linux desktop x86_64, macOS universal and Windows artifact preflight builds validate checksums/SBOM, setup, agent health, MCP initialize, an owner-approved write, desktop launch where applicable, uninstall and residue cleanup. A physical Apple-silicon Mac additionally verifies both universal slices through Rosetta/native launch, DMG installation, LaunchAgent stop/start, all seven desktop navigation views, retained-data uninstall and full purge. A physical Ubuntu 24.04 x86_64 Xfce host verifies the full desktop-control build, four-binary SBOM/archive, standalone DEB install/remove/residue cycle, installed MCP lifecycle, all seven rendered views, a real StatusNotifierItem, window-manager close-to-tray, tray re-open, and clean tray removal. Windows preflight now requires a real native window, the same seven-view contract, `WM_CLOSE` hide-to-tray behavior, current-user NSIS install, HKCU/shortcut verification, silent uninstall, and residue cleanup; GNU/Wine evidence remains supplemental rather than physical Windows proof. The strict release gate still requires actual OS reboots, Windows publisher signing, and Apple Developer ID signing/notarization.
