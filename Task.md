...
- [!] **P2-R03 — Complete clean-install acceptance on every artifact/OS.** Fresh hosted Linux x86/ARM, macOS universal and Windows artifact preflight builds validate checksums/SBOM, setup, agent health, MCP initialize, an owner-approved write, desktop launch where applicable, uninstall and residue cleanup. A physical Apple-silicon Mac additionally verifies both universal slices through Rosetta/native launch, DMG installation, LaunchAgent stop/start, all desktop navigation views, retained-data uninstall and full purge. The strict release evidence gate still requires an actual OS reboot plus Developer ID signing and notarization.
